                                     ...
                                     <div class="table-responsive">
                                                    <table class="table m-0">
                                                        <thead class="thead-light">
                                                            <tr>
                                                                ...
                                                                <th><i class="fa fa-dollar"></i> User Earnings</th>
                                                                <th><i class="fa fa-users"></i> Referral Earnings</th>
	                                           ...
	                                           {foreach from=$topusers item=foo}
                                                            <tr>
                                                                ...
                                                                <td>{$foo['username']}</td>
                                                                <td>{$foo['balance']} <b>{$sumbole}</b></td>
                                                                <td>{$foo['ref']} <b>{$sumbole}</b></td>
                                                                <td>
                                               {if $foo['status'] == 1}
                    ...
                    <td>
                     <form method="POST" action="{$ADM}users/edit/{$foo['id']}/">
                      <button class="btn btn-success btn-xs" type="submit">Edit</button>
                                               ...
                                               {if !$topusers}<span class="badge badge-danger elem-center" style="margin-left: 200px;">No top users was Found..</span>{/if}
                               </div>
                            </div>
                        </div>
                        <!-- end row -->
